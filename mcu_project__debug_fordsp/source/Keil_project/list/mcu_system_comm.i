# 1 "../../mcu/mcu_system_comm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 389 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../../mcu/mcu_system_comm.c" 2
# 12 "../../mcu/mcu_system_comm.c"
# 1 "../../mcu/mcu_system_comm.h" 1
# 14 "../../mcu/mcu_system_comm.h"
...
# 19 "../../mcu/mcu_system_comm.c" 2
# 37 "../../mcu/mcu_system_comm.c"
void fh_delay_ms(uint32_t ms)
{
 uint32_t i = 0;
 uint32_t j = 0;
...
# 85 "../../mcu/mcu_system_comm.c"
void system_delay_ms(uint32_t ms)
{

 uint32_t tiks = g_mcu_vars_param.g_sys_tick_period + ms;
 ...
 while(g_mcu_vars_param.g_sys_tick_period < tiks)
 {
...
# 117 "../../mcu/mcu_system_comm.c"
uint32_t system_get_current_ticks()
{
 return g_mcu_vars_param.g_sys_tick_period;
}
# 136 "../../mcu/mcu_system_comm.c"
void set_u16( uint8_t *buf, uint16_t val)
{
...
# 222 "../../mcu/mcu_system_comm.c"
uint32_t get_u32(uint8_t *pVal)
{
    uint32_t val = 0;
    uint8_t firstByteValue = 0;
    uint8_t secondByteValue = 0;
    uint8_t thirdByteValue = 0;
    uint8_t fourthByteValue = 0;
    ...
    firstByteValue = *(pVal);
    secondByteValue = *(pVal + 1);
    thirdByteValue = *(pVal + 2);
    fourthByteValue = *(pVal + 3);
 ...
 uint32_t* temp)
{
 ...
 if (leftn == rightn)
 {
  *temp = (leftn * scale) * 100;

 }
 else
 {
  *temp = (leftn * scale) * 100 +
   (*(tec_table + leftn) - adc_value) * 100 * scale / (*(tec_table +
   leftn) - *(tec_table + rightn));
 }

 return;
}
# 365 "../../mcu/mcu_system_comm.c"
uint8_t search_bisection_temp_by_adc(uint16_t adc_value,
   uint16_t* tec_table,
   uint16_t tec_count,
   uint8_t scale,
...
int32_t flash_write(uint8_t *inputdata,uint32_t addr_start,uint16_t data_len)
{
 ...
 uint64_t data=0;
 new_addr = addr_start;
 fmc_bank0_flag_clear(((uint32_t)((uint32_t)0x01U<<(5))) | ((uint32_t)((uint32_t)0x01U<<(4))) | ((uint32_t)((uint32_t)0x01U<<(2))));
    fmc_unlock();
 for(i=0;i<data_len;i++)
 {
  if((i+1) % 8 == 0){
...
void fmc_erase_single_page(uint32_t start_addr)
{
    ...
    fmc_bank0_flag_clear(((uint32_t)((uint32_t)0x01U<<(5))) | ((uint32_t)((uint32_t)0x01U<<(4))) |
