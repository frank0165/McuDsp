# 1 "../../update/update_comm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 389 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../../update/update_comm.c" 2
# 13 "../../update/update_comm.c"
...
# 46 "../../update/update_comm.c"
ErrStatus update_mem_malloc()
{
 if(0U == g_update_mem_ptr)
 {
  g_update_mem_ptr = (uint8_t *)malloc(1024 + 200);
  if(g_update_mem_ptr == 0)
  {
 ...
 if(g_update_mem_ptr != 0)
 {
  memset(g_update_mem_ptr,0,1024);
 }
 else
 {
  return ERROR;
...
# 150 "../../update/update_comm.c"
void read_update_flag()
{
 uint32_t addr_start = (0x0807F000U - 0x1400) + 7 * sizeof(uint8_t);
 flash_read(&update_flag,addr_start,1);

}
# 169 "../../update/update_comm.c"
void write_update_flag(uint64_t newflag)
{
 uint8_t inputdata[8];
 ...
 fmc_erase_single_page((0x0807F000U - 0x1400));
 flash_write(inputdata,(0x0807F000U - 0x1400),8);

}
# 195 "../../update/update_comm.c"
ErrStatus comm_update_flash_finish_one_page(uint32_t upate_addr)
{


 fmc_erase_single_page(upate_addr + g_page_idx * 1024);
 flash_write(g_update_mem_ptr,
  upate_addr + g_page_idx * 1024,
  1024);

 g_page_idx++;
 g_pkt_count_to_flash += 1024;

 return SUCCESS;
}
# 222 "../../update/update_comm.c"
void comm_update_write_to_flash(uint32_t upate_addr)
{
 uint32_t i = 0;
 uint32_t div_mod = 0;
 ...
 div_mod = g_update_mem_idx % 8;

 if(0 != div_mod)
 {

  if((g_update_mem_idx + (8 - div_mod)) > 1024)
  {
   return;
  }


  for(i = 0; i < (8 - div_mod);i++)
  {
  ...
  uint8_t *pbuffer,uint32_t buffer_len)
{
 ...
 if((g_update_mem_idx + buffer_len) <= 1024)
 {
 ...
 else
 {
  ...
  memcpy(&g_update_mem_ptr[g_update_mem_idx],
   pbuffer,1024 - g_update_mem_idx);

  cur_idx = 1024 - g_update_mem_idx;
...
# 357 "../../update/update_comm.c"
void test_dac_value_write_reg(uint8_t *p_buff,uint32_t buff_len)
{
 uint8_t k = 0;
 uint8_t tmp1 = 0;
 uint8_t tmp2 = 0;
 uint32_t dac_perith = (((uint32_t)0x40000000U) + 0x00007400U);
 uint32_t dac_out = ((uint8_t)0x00U);
 ...
 switch(p_buff[k])
 {
  case 0:
  {
   dac_perith = (((uint32_t)0x40000000U) + 0x00007400U);
   break;
  }

  case 1:
  {
   dac_perith = (((uint32_t)0x40000000U) + 0x0000CC00U);
   break;
  }
  default:
  {
 ...
 switch(p_buff[k])
 {
  case 0:
  {
   dac_out = ((uint8_t)0x00U);
   break;
  }

  case 1:
  {
   dac_out = ((uint8_t)0x01U);
   break;
  }
  default:
  {
 ...
 dac_data_set(dac_perith, dac_out,(((0xFFFFFFFFUL << (0)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(1)))) & ((uint32_t)(0) << 0)),dac_value);
...
uint32_t test_get_gpio_periph(uint32_t flg)
{
 ...
 switch(flg)
 {
  case 0:
  {
   gpio_periph = ((((uint32_t)0x48000000U) + 0x00000000U) + 0x00000000U);
   break;
  }
  case 1:
  {
   gpio_periph = ((((uint32_t)0x48000000U) + 0x00000000U) + 0x00000400U);
   break;
  }
  case 2:
  {
   gpio_periph = ((((uint32_t)0x48000000U) + 0x00000000U) + 0x00000800U);
   break;
  }
  case 3:
  {
   gpio_periph = ((((uint32_t)0x48000000U) + 0x00000000U) + 0x00000C00U);
   break;
  }
  case 4:
  {
   gpio_periph = ((((uint32_t)0x48000000U) + 0x00000000U) + 0x00001400U);
   break;
  }
  default:
  {
...
uint32_t test_get_gpio_pin(uint32_t flag)
{
 ...
 if(flag < 15)
 {
  pin = ((uint32_t)((uint32_t)0x01U<<(flag)));
 }
 return pin;
}
...
void test_gpio_value_write_reg(uint8_t *p_buff,uint32_t buff_len)
{
 ...
 if(((((uint32_t)0x48000000U) + 0x00000000U) + 0x00000400U) == gpio_periph && ((uint32_t)((uint32_t)0x01U<<(3))) == pin)
 {
  gpio_mode_set(gpio_periph, (((0xFFFFFFFFUL << (0)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(1)))) & ((uint32_t)(1) << 0)), (((0xFFFFFFFFUL << (0)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(1)))) & ((uint32_t)(0) << 0)), ((uint32_t)((uint32_t)0x01U<<(3))));
 }
